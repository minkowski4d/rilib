...
def irrbb_shock_scenarios(df_orig):
    ...
    df['delta_eve'] = df[delta_eve_cols].groupby('bucket').sum(numeric_only=True).T.min().min()
    ...
    df['delta_nii'] = df[delta_nii_cols].groupby('bucket').sum(numeric_only=True).T.min().min()
