...
def build_trade_cache_eod_close(enddate=None):
    ...
    # Get Trades
    df_trades = get_trades_pnl_daily_reset(startdate=date(2022, 9, 1), enddate=enddate)
...
def get_prices_eod(syms=None, startdate=None, enddate=None, read_price_cache=False, read_full_dwh=False, fill_na=True):
    """
    Function to retrieve EOD Prices out of TEAMS_PRD.RISK_FUNCTION_PUBLISH.pbl__risk_function_mrm_book_msl_prices_tib_strat
    Prices are EUR-converted using EURRATE and dated in Europe/Berlin timezone.
    ...
    if read_price_cache:
        ...
        if read_full_dwh:
            ...
            qry_prices = qry % {
                "start": db.sqldate(startdate - timedelta(3)),
                "end": db.sqldate(enddate),
            }
            ...
            qry = '''
                    SELECT
                        convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(CLOSETIMESTAMP/1e9))::DATE AS ddate,
                        ISIN AS symbol,
                        CLOSEPRICE * EURRATE AS price
                    FROM TEAMS_PRD.RISK_FUNCTION_PUBLISH.pbl__risk_function_mrm_book_msl_prices_tib_strat
                    WHERE CLOSEPRICE IS NOT NULL
                      AND EURRATE IS NOT NULL
                      AND CLOSESIZE <> 0
                      AND convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(CLOSETIMESTAMP/1e9))::DATE BETWEEN %(start)s AND %(end)s
                      AND ISIN IN (%(syms)s)
                    QUALIFY ROW_NUMBER() OVER (PARTITION BY convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(CLOSETIMESTAMP/1e9))::DATE, ISIN ORDER BY CLOSESIZE DESC) = 1
                    ORDER BY 1,2
                    '''
            qry_prices = qry % {
                "start": db.sqldate(startdate - timedelta(3)),
                "end": db.sqldate(enddate),
                "syms": db.joinpad(syms),
            }
...
def get_trades_pnl_daily_reset(account=None, startdate=None, enddate=None, syms=None, verbose=False):
    """
    Trades Query for PnL Purpose that Picks up accounts and adds sec infos

    @param account: trading account / sec_acc_no
    @param syms: optional list of ISINs
    @param startdate: date object
    @param enddate: date object
    ...
    sym_filter_sql = ""
    if syms is not None:
        sym_filter_sql = f"AND isin IN ({_sql_in_list(syms)})"
    
    if account == 9800001601:
        account_tib= 200 
    elif account == 9800003601:
        account_tib= 600
    else:
    ...
    tr_query = f"""
        WITH
        corrected_trade_keys AS (
            -- Original trades that were later cancelled or price-corrected (from executions system)
            SELECT DISTINCT
                orig.ISIN,
                orig.TRADING_ACCOUNT,
                orig.SEQ_DATE                               AS original_date,
                orig.PRICE                                  AS original_price,
                IFF(orig.SIDE = 'Sell', -1, 1) * orig.QTY  AS signed_qty
            FROM TEAMS_PRD.MM_MART.MRT__MM__EXECUTIONS orig
            INNER JOIN TEAMS_PRD.MM_MART.MRT__MM__EXECUTIONS corr
                ON  orig.SEQ_NO          = corr.LAST_CORRECTED_OR_CANCELLED_SEQ_NO
                AND orig.SEQ_DATE        = corr.ORIG_TRANSACT_TIME::DATE
                AND orig.TRADING_ACCOUNT = corr.TRADING_ACCOUNT
            WHERE corr.CANCELLED = true
              AND corr.TRADING_ACCOUNT = {account_tib}
        ),
        correction_events AS (
            -- For each corrected original, locate the simultaneous CANCELLED+CORRECTED pair
            -- in enriched trades and extract the new price.
            -- A pure cancellation has no CORRECTED counterpart and is absent from this CTE.
            SELECT
                ctk.ISIN,
                ctk.TRADING_ACCOUNT,
                ctk.original_date,
                ctk.original_price,
                ctk.signed_qty,
                cor.PRICE AS new_price
            FROM corrected_trade_keys ctk
            INNER JOIN TIBERIUS_PRD.TRADING_PLATFORM_EXTERNAL_SOURCES.MM_INQUISITOR_ENRICHED_TRADES can_e
                ON  can_e.ISIN           = ctk.ISIN
                AND can_e.TRADINGACCOUNT = ctk.TRADING_ACCOUNT
                AND can_e.PRICE          = ctk.original_price
                AND can_e.NOTIONAL       = -ctk.signed_qty
                AND can_e.SOURCE         = 'CANCELLED'
            INNER JOIN TIBERIUS_PRD.TRADING_PLATFORM_EXTERNAL_SOURCES.MM_INQUISITOR_ENRICHED_TRADES cor
                ON  cor.ISIN             = can_e.ISIN
                AND cor.TRADINGACCOUNT   = can_e.TRADINGACCOUNT
                AND cor.TIMESTAMP        = can_e.TIMESTAMP
                AND cor.NOTIONAL         = -can_e.NOTIONAL
                AND cor.SOURCE           = 'CORRECTED'
            QUALIFY ROW_NUMBER() OVER (
                PARTITION BY ctk.ISIN, ctk.TRADING_ACCOUNT, ctk.original_date, ctk.original_price, ctk.signed_qty
                ORDER BY can_e.TIMESTAMP DESC
            ) = 1
        ),
        all_enriched AS (
            SELECT
                e.*,
                convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(e.TIMESTAMP/1e9))::DATE AS trade_date
            FROM TIBERIUS_PRD.TRADING_PLATFORM_EXTERNAL_SOURCES.MM_INQUISITOR_ENRICHED_TRADES e
            WHERE e.TRADINGACCOUNT = '{account_tib}'
              AND convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(e.TIMESTAMP/1e9))::DATE >= {db.sqldate(startdate)}
              AND convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(e.TIMESTAMP/1e9))::DATE <= {db.sqldate(enddate)}
              {sym_filter_sql}
        ),
        raw AS (
            -- Clean base trades: exclude all cancelled/corrected originals and all
            -- CANCELLED/CORRECTED rows (no netting rows in the FIFO stream)
            SELECT
                e.*,
                ROW_NUMBER() OVER (ORDER BY e.TIMESTAMP) AS row_id
            FROM all_enriched e
            LEFT JOIN corrected_trade_keys ctk
                ON  e.ISIN           = ctk.ISIN
                AND e.TRADINGACCOUNT = ctk.TRADING_ACCOUNT
                AND e.PRICE          = ctk.original_price
                AND e.NOTIONAL       = ctk.signed_qty
                AND e.trade_date     = ctk.original_date
            WHERE e.SOURCE NOT IN ('CANCELLED', 'CORRECTED')
              AND ctk.ISIN IS NULL
        ),
        price_corrected_trades AS (
            -- Re-inject price-corrected originals at their original timestamp with the new
            -- price so FIFO sees the right cost basis at the right point in time
            SELECT
                e.* REPLACE (ce.new_price AS PRICE),
                NULL::BIGINT AS row_id
            FROM all_enriched e
            INNER JOIN correction_events ce
                ON  e.ISIN           = ce.ISIN
                AND e.TRADINGACCOUNT = ce.TRADING_ACCOUNT
                AND e.PRICE          = ce.original_price
                AND e.NOTIONAL       = ce.signed_qty
                AND e.trade_date     = ce.original_date
            WHERE e.SOURCE NOT IN ('CANCELLED', 'CORRECTED')
        ),
        prefinal AS (
            SELECT * FROM raw
            UNION ALL
            SELECT * FROM price_corrected_trades
        ),
        base_trades AS (
            SELECT
                convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(TIMESTAMP/1e9))::TIMESTAMP_NTZ as trade_ts,
                convert_timezone('UTC','Europe/Berlin',TO_TIMESTAMP(TIMESTAMP/1e9))::DATE as report_date,
                isin as instrument_id,
                IFF(NOTIONAL <0,'S','B') as side,
                ORDERTYPE as booking_type,
                PRICE as price,
                ABS(NOTIONAL) as quantity,
                NOTIONAL as quantity_signed,
                1 as multiplier,
                SOURCE as source,
                TRADINGACCOUNT
            FROM prefinal
        ...
        SELECT trade_ts,
            ...
            multiplier,
            source,
            'ENRICHED_TRADES' as data_src_primary
        FROM base_trades
        WHERE
            -- keep real and virtual trades
            source <> 'SYNTHETIC'

            OR

            -- keep ONLY first synthetic per ISIN within the window (inventory injection on day 1)
            trade_ts = (
                SELECT MIN(trade_ts)
                FROM base_trades b2
                WHERE b2.instrument_id = base_trades.instrument_id
                AND b2.source = 'SYNTHETIC'
            )
    ...
    # Drop SYNTHETIC closes (23:59:59) — these are system outputs, not FIFO inputs.
    # For ISINs whose first SYNTHETIC in history is a close (no prior inventory),
    # we start FIFO from zero (empty), which is equivalent to SYNTHETIC_OPEN = 0.
    df_trades = df_trades[
        ~((df_trades['source'] == 'SYNTHETIC') &
          (pd.to_datetime(df_trades['trade_ts']).dt.time == pd.Timestamp('23:59:59').time()))
    ]
    ...
    df_trades = df_trades[
        ['trade_ts', 'report_date', 'instrument_id', 'side',
    ...
    df_trades.columns = [
        'time', 'booking_date', 'symbol', 'side',
        'booking_category', 'booking_type',
        'price', 'quantity', 'quantity_signed',
        'multiplier', 'data_src_primary'
    ]
    # source was only needed for the SYNTHETIC-close filter above; not passed downstream
...
def build_initial_cache(syms=None, startdate=None, enddate=None, verbose=True):
    ...
    df_trades = get_trades_pnl_daily_reset(syms=syms, startdate=startdate + timedelta(1),enddate=enddate)
...
def pnl2parquet(acct=None,df_orig=None,report_date=None,pth=None,verbose=True, **kwargs):
    ...
    if df_orig.empty:
        logging.warning(f"pnl2parquet: empty df_orig for account {acct}, report_date {report_date} — skipping save")
        return None
    ...
    if 'cache' in kwargs:
        ...
        filename = os.path.join(pth,'%s_%s_%s_cache_daily_reset.parquet'%(report_date.strftime("%Y%m%d"),str(acct),str(data_src_primary)))
        ...
        filename= os.path.join(pth,'%s_%s_%s_pnl_daily_reset.parquet'%(report_date.strftime("%Y%m%d"),str(acct),str(data_src_primary)))
