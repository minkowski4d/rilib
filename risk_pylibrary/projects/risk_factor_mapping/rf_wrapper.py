...
import pandas as pd
from risk_pylibrary.projects.risk_factor_mapping import rf_wrapper_support as rfws
from tools.snowflake_db import db_connection as db




def rf_mapping_engine(ddate, ddate_pos, verbose=True):
    ...
    df_orig = rfws.rf_get_secs_info(ddate, ddate_pos, verbose=True)
    ...
    out_dict = rfws.rf_stocks_country_dict()
    ...
    df = bonds_mapping_risk_factors(df, 1)
...
def bonds_mapping_risk_factors(df_orig, verbose):
    ...
    df['risk_factor'] = np.nan
    df['oas_series'] = np.nan
    unmapped_ids = []
    for j in df.index:
