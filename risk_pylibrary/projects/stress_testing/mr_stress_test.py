...
def stress_test_engine(ddate=None, port='caracalla', stress_scenarios=None, horizon=250, apply_dq=True, plot=False, verbose=True):
    ...
    if port in ['caracalla', 'caligula', 'tiberius']:
        ...
        if len(CF.cache_caracalla_port) == 0:
            ...
            if ddate:
                out_dict_pf = c_port.get_caracalla_portfolio(value_date=ddate,
                                                             force_rf=False,
                                                             calc_rf_port=True,
                                                             calc_pnl_report=False,
                                                             rf_mapping_new=True,
                                                             new_pf_engine=True, live_trading=False, account=port, verbose=False)
            else:
                out_dict_pf = c_port.get_caracalla_portfolio(force_rf=True,
                                                             calc_rf_port=True,
                                                             calc_pnl_report=False,
                                                             rf_mapping_new=False,
                                                             new_pf_engine=True, live_trading=False, account=port, verbose=False)
            #CF.cache_caracalla_port = out_dict_pf
        else:
            out_dict_pf = CF.cache_caracalla_port
    ...
    sf_stress_dict['dur_mod'] = sf_stress_dict.index.get_level_values(0)
    sf_stress_dict['dur_mod'] = sf_stress_dict['dur_mod'].map(dur_mod_map)
    ...
    # Percentage Returns
    sf_list_pct = sf_stress_dict[sf_stress_dict.dur_mod.isnull()].index.get_level_values(0)
